...
import { AdminReferenceDataTab } from './AdminReferenceDataTab'
import { AdminAiTab } from './AdminAiTab'
import { AdminFeatureFlagsTab } from './AdminFeatureFlagsTab'
import { AdminLogTab } from './AdminLogTab'

const TABS = [
  { id: 'dashboard', label: '🏠 الرئيسية' },
  { id: 'clubs', label: '🏢 الأندية المشتركة' },
  { id: 'reference', label: '🆚 الأندية والمسابقات' },
  { id: 'ai', label: '🤖 الذكاء الاصطناعي' },
  { id: 'flags', label: '🚩 الميزات' },
...
export function AdminScreen() {
  const { isAdmin, adminChecked, signOut } = useAuthStore()
  ...
  return (
    <div className="p-4 animate-[fadeIn_.3s_ease]">
      ...
      {tab === 'reference' && <AdminReferenceDataTab />}
      {tab === 'ai' && <AdminAiTab />}
